class Staff::CustomerSearchForm
  ...
  attr_accessor :family_name_kana, :given_name_kana,
    ...
    :last_four_digits_of_phone_number, :gender, :postal_code
  ...
  def search
    ...
    rel = rel.joins(:phones).where('phones.number_for_index' => phone_number) if phone_number.present?
    if last_four_digits_of_phone_number.present?
      rel = rel.joins(:phones)
        .where('RIGHT(phones.number_for_index, 4) = ?', last_four_digits_of_phone_number)
    end
